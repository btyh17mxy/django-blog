...
class SiteInfo(models.Model):
    ...
    sub_title = models.CharField(max_length=240, verbose_name=u'副标题')
    keywords = models.CharField(max_length=240, verbose_name=u'关键字')
    description = models.CharField(max_length=240, verbose_name=u'描述')
    ...
    def get_siteinfo(cls):
        site = cls.objects.all()
        if len(site)  == 0:
            return {
                    'title':u'个人博客系统',
                    'sub_title':u'http://mushapi.sinaapp.com',
                    'keywords':u'Django,Python,Blog',
                    'description':u'url',
                    }
        else:
            return site[0]
    ...
    class Meta:
        verbose_name_plural = verbose_name = u"站点信息"
...
class Post(models.Model):
    ...
    content = models.TextField(verbose_name=u'文章正文内容')
    #content = UEditorField(u'内容	',height=100,width=500,default='test',imagePath="post/",imageManagerPath="imglib",toolbars='full',options={"elementPathEnabled":True},filePath='post/',blank=True)
    ...
    style = models.IntegerField(default=0, choices=STYLE.items(), verbose_name=u'类型,目前只支持html')
